...
<div class="wrap">
    ...
    <h2><?php _e('Cyclone Slider Exporter', 'cycloneslider'); ?></h2>
	...
	<form method="post" action="<?php echo esc_url( $export_page_url ); ?>">
        ...
        <?php if($sliders): ?>
            <table class="form-table">
                <tr>
                    <th><h4><?php _e('Select sliders:', 'cycloneslider'); ?></h4></th>
                    <td>
                        ...
                        <?php foreach($sliders as $slider): ?>
                            <label for="cs-slider-<?php echo $slider['name']; ?>">
                                <input class="cs-sliders" type="checkbox" id="cs-slider-<?php echo esc_attr( $slider['name'] ); ?>" name="<?php echo esc_attr( $transient_name ); ?>[sliders][]" value="<?php echo esc_attr( $slider['name'] ); ?>" <?php echo ( in_array($slider['name'], $page_data['sliders']) ) ? 'checked="checked"' : '' ; ?> />
                                <span><em><?php echo esc_html($slider['title']); ?></em></span>
                            </label> <br />
                        <?php endforeach; ?>
                    </td>
                </tr>
